...
`api::streams::StreamGate` is a subscriber count with an `on_first` and an
`on_last` hook. `AppState::multiview_gate` is one. Whoever owns a pipeline
that should only run while somebody is watching sets those two hooks at
startup and never thinks about WebSockets.
