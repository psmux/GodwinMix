...
use crate::api::streams::StreamGate;
...
pub struct AppState {
    ...
    /// Mosaic frames as they come off the encoder. `None` when multiview is
    /// disabled.
    pub frames: Option<Arc<broadcast::Sender<Arc<[u8]>>>>,
    /// The same frames with the 16 byte header on the front, built once for
    /// every `/rpc` client rather than once per client.
    pub headed_frames: Arc<broadcast::Sender<Arc<[u8]>>>,
    ...
    pub rehearsal: bool,
    /// How many clients want the mosaic. Whoever owns multiview.rs hangs the
    /// build and tear down of the pipeline off this; see `api::streams`.
    pub multiview_gate: Arc<StreamGate>,
}
...
impl AppState {
    ...
    pub fn new(
        ...
        let (headed_frames, _) = broadcast::channel(2);
        Self {
            mixer,
            frames,
            headed_frames: Arc::new(headed_frames),
            library,
            converter,
            quit,
...
pub async fn snapshot_bytes(
    ...
    if !snapshots.enabled() {
        return Err(RpcError::not_in_state(
            "multiview is disabled on this mixer, so there is no mosaic to cut a snapshot \
             out of. Set `enabled = true` under [multiview] and restart.",
        ));
    }
    let Some(latest) = snapshots.latest() else {
        return Err(RpcError::not_in_state(
            "no mosaic frame has arrived yet. Wait for a source to go live and try again.",
        ));
    };
...
struct SnapshotQuery {
    /// Downscale to this many pixels across, keeping the aspect.
    #[serde(default)]
    width: Option<u32>,
}
...
async fn snapshot_image(
    ...
    match snapshot_bytes(&snapshots, &name, q.width).await {
        Ok(jpeg) => (
            StatusCode::OK,
...
/// Put the 16 byte header on every mosaic frame, once, for every `/rpc`
/// client at the same time.
///
/// Parked while nobody is watching, so a headless core with no UI attached
/// does not even copy a frame. The mosaic pipeline itself is started and
/// stopped by whoever owns multiview.rs, off the same gate.
fn spawn_frame_relay(app: AppState) {
    let Some(frames) = app.frames.clone() else { return };
    tokio::spawn(async move {
        let mut clock = RunningTime::default();
            ...
            while app.multiview_gate.anyone_watching() {
                tokio::select! {
                    event = events.recv() => match event {
                        Ok(envelope) => clock.observe(&envelope.event),
                    ...
                    frame = rx.recv() => match frame {
                        Ok(jpeg) => {
                            ...
                            let header = crate::api::rpc::frame_header(
                                counter,
                                clock.layout,
                                clock.now_ms(),
                            );
                            ...
                            let _ = app.headed_frames.send(out.into());
                        }
                        Err(broadcast::error::RecvError::Closed) => return,
                        Err(broadcast::error::RecvError::Lagged(n)) => {
                            debug!(skipped = n, "frame relay fell behind the mosaic");
                        }
                    },
                }
            }
