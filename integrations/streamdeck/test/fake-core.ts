...
export interface FakeCore {
  ...
  calls: Array<{ method: string; params: Record<string, unknown>; id?: number }>;
...
export async function fakeCore(
  opts: { onSubscribe?: (core: FakeCore) => void } = {},
): Promise<FakeCore> {
  ...
  const core: FakeCore = {
    ...
    notify(name, params) {
      ...
      for (const socket of sockets) socket.write(frame(0x1, Buffer.from(body)));
    },
    frame(seq, layout, runningTimeMs, jpeg) {
      ...
      const message = Buffer.concat([header, Buffer.from(jpeg)]);
      for (const socket of sockets) socket.write(frame(0x2, message));
    },
    answer(method, reply) {
  ...
  server.on("upgrade", (req, socket: Duplex) => {
    ...
    read(socket, (opcode, payload) => {
      ...
      calls.push({ method: call.method, params: call.params || {}, id: call.id });
      ...
      if (call.method === "core.subscribe") opts.onSubscribe?.(core);
    });
  });
