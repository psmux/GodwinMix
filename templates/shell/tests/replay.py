...
Bytes in, bytes out. It spawns `sh run.sh`, writes the `core` lines to its stdin
...
This is the only Python in the template, and it is a test, not the plugin. A
shell script cannot drive another process, hold a ten second deadline and
compare JSON without more `sed` than anyone should read. python3 is needed to
run the tests and is not needed to run the plugin.
...
def main():
    ...
    shell = os.environ.get("GMX_SH", "sh")
    plugin = subprocess.Popen(
        [shell, os.path.join(here, "run.sh")],
