...
def read(path):
    ...
    plugin, run, provides, table = {}, {}, [], None
...
def main():
    ...
    # One runtime key, and for this template it is the shell one. The Windows
    # rule below is the core's, not ours: a shell entry is refused there unless
    # [run] also carries a bin entry, so this template does not claim Windows.
    if len(run) != 1:
        problems.append("run: set exactly one of bin, python, node or shell. "
                        "This has %d" % len(run))
    elif "shell" not in run:
        problems.append("run.%s: this template is started by sh. Use "
                        "shell = \"run.sh\", or rewrite the plugin for that "
                        "runtime." % list(run)[0])
    elif not os.path.exists(os.path.join(root, run["shell"].strip('"'))):
        problems.append("run.shell: '%s' does not exist" % run["shell"].strip('"'))
    if "shell" in run and "bin" not in run and "windows" in plugin.get("platforms", ""):
        problems.append("plugin.platforms: a shell entry is refused on Windows "
                        "unless [run] also has a bin entry for it. Drop windows "
                        "from plugin.platforms, or add the binary.")
