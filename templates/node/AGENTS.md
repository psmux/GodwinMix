...
A GodwinMix source plugin in Node. One process, started by the core. Control is
...
Nothing outside the Node 20 standard library is required, there is no
`package.json`, and the check script runs on a bare machine. Keep it that way
unless the plugin genuinely needs a library, and if it does, add a
`package.json` with the dependency pinned so `gmx plugin add` can install it
into the plugin's own directory.

## Build and test

```sh
./check            # syntax, the manifest, the offline transcript, your test. Under 30 s.
...
| Path | What it is |
|---|---|
| `main.js` | the whole plugin. `draw()` is the part you change |
...
| `tests/replay.js` | the replayer. Do not edit it to make a test pass |
| `tests/check_manifest.js` | a rough manifest check for machines with no `gmx` |
| `tests/test_picture.js` | the failing test. Replace it |
| `check` | everything above, in order |

## The rules that matter

1. **stdout is media.** A `console.log()` anywhere in this process corrupts the
   video stream. Log with the `log()` helper, which writes to stderr. The check
   greps `main.js` for `console.log` and fails on it.
2. **`draw()` must return exactly `width * height * 3 / 2` bytes** for the canvas
   the core sent. A wrong length stops the media loop and logs both numbers.
3. **`draw()` runs on the event loop.** No synchronous network calls, no
   `readFileSync`, no long loops. Do that work elsewhere and hand the result
   over through `state.params` or a variable the next frame reads.
4. **`health` must answer while `start` is still running.** The template answers
   it from the readline handler without touching the frame timer; keep that
   shape.
5. **`configure` gets the full validated object, not a diff.** Store it; the next
   frame reads it.
6. **Bitwise operators are 32 bit.** `vint()` builds sizes with division and a
   small shift for that reason. A 1080p frame needs a four byte size.
7. **Back pressure is real.** `MEDIA.write()` returns false when the pipe is
   full; the template waits for `drain` rather than queueing frames in memory.
   If you change the pacing, keep that.
...
* Do not edit `tests/replay.js` or `tests/transcript.jsonl` to make a failing
