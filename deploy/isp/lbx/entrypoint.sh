...
if [ -f /etc/liveboxmix/wpe.toml ]; then
  mkdir -p /var/lib/liveboxmix/media-wpe
  ...
  PATH=/usr/bin:/bin WEBKIT_DISABLE_SANDBOX_THIS_IS_DANGEROUS=1 GST_GL_PLATFORM=egl GST_GL_API=gles2 \
    /opt/lbx/wpe/liveboxmix --config /etc/liveboxmix/wpe.toml > /var/log/liveboxmix-wpe.log 2>&1 &
fi
exec /opt/lbx/bin/liveboxmix --config /etc/liveboxmix/liveboxmix.toml
