...
pkill -f "lavfi -i smptebars"; pkill -f "http.server 8090"; pkill -f "liveboxmix-browser"
...
if [ -n "$DOCKER_HOST" ] || [ -S "$HOME/.colima/default/docker.sock" ]; then
  ...
  docker ps -q --filter name=lbx-browser- 2>/dev/null | xargs -r docker rm -f >/dev/null 2>&1
fi
true
