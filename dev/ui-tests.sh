...
# designer kits against their fixtures), then the live suite drives the real
# Scenes panel and the real composer over `/rpc`: two inputs dragged onto empty
# space, F2, a colour, a take, a layout pasted onto another scene, a drag in the
# composer with its timings, Apply, Delete and undo.
#
...
(cd "$WORK" && GMX_UI_DEV=1 "$REPO/target/debug/godwinmix" --config "$WORK/godwinmix.toml") >"$LOG" 2>&1 &
CORE_PID=$!
