...
fn the_recorded_transcript_replays() {
    ...
    let mut plugin = Command::new(env!("CARGO_BIN_EXE_gmx-camera"))
        .current_dir(root)
        .env("GMX_PLUGIN", "camera")
...
///
/// This camera captures with avfvideosrc, v4l2src and the Media Foundation sources, and every one of them stamps its
