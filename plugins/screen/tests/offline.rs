...
fn the_recorded_transcript_replays() {
    ...
    let mut plugin = Command::new(env!("CARGO_BIN_EXE_gmx-screen"))
        .current_dir(root)
        .env("GMX_PLUGIN", "screen")
...
///
/// This screen captures with the desktop capture sources on every platform, and every one of them stamps its
