#!/usr/bin/env python3
"""A GodwinMix source plugin that imports nothing outside the standard library.

It draws eight colour bars at the canvas caps and writes them to stdout as raw
I420 frames in a streamable Matroska stream. Control is JSON-RPC 2.0, one
object per line, on stdin and stderr. Run ./examples/test-zero-dep.sh to drive
it with no core. This file keeps the protocol honest: if it ever needs a
dependency, the protocol has drifted and the protocol is wrong, not this file.
"""
import json
import struct
import sys
...
state = {"canvas": None, "params": {}, "stop": threading.Event(),
         "thread": None, "frames": 0, "cluster_ms": None, "header": False}
...
# --- EBML, the little of it a stream needs, then the picture, then the calls -
...
def write_header(canvas):
    ...
    info = (elem(b"\x2a\xd7\xb1", uint(TIMECODE_SCALE_NS))
            + elem(b"\x4d\x80", b"zero-dep\x00") + elem(b"\x57\x41", b"zero-dep\x00"))
...
def colour_bars(canvas):
    """One I420 frame of bars, built once and sent again every frame."""
    width, height = canvas["width"], canvas["height"]
    cw, ch = (width + 1) // 2, (height + 1) // 2
    def bar(x):
        return BARS[min(x * 8 // width, 7)]
...
def on_start(rid, params):
    ...
    if transport != "container":
        return fail(rid, -32602, "this plugin only speaks the container transport. "
                    "Give it transports = [\"container\"] in the core's config.",
...
def dispatch(message):
    ...
    if method == "start":
        ...
        fail(rid, -32601, "this plugin has no method '%s'. It implements start, stop, "
             "configure, health and shutdown." % method,
...
def main():
    ...
    send({"jsonrpc": "2.0", "id": 0, "method": "initialize",
          "params": {"plugin": "zero-dep", "version": "0.1.0", "api": API,
    ...
    for line in sys.stdin:
        ...
        if message.get("id") == 0 and "result" in message:
            ...
            log("info", "zero-dep source at %dx%d@%d"
