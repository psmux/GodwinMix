...
export function startRecording(client) {
  ...
  start.onclick = async () => {
    ...
    try {
      ...
      dialog.close();
      toast({ text: "Recording started. Outputs shows the file and its status." });
    } catch (e) {
...
export function recordingRow(client, output) {
  const active = output.state === "live";
  return el("div.output-row.recording-row", {}, [
    el("div.row", {}, [
      el("span.dot." + (active ? "live" : "failed")),
      el("strong", { text: active ? "Recording" : "Recording needs attention" }),
      el("span.grow"),
