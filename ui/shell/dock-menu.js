...
import { savedLayouts } from './dock-presets.js';

export function workspaceMenu(workspace) {
  const hidden = new Set(workspace.state.hidden);
  const body = el('div.col.dock-manager', {}, list().filter(s => !s.slots.includes('header') && !s.slots.includes('modal')).map(spec =>
    el('button.btn', { text: (hidden.has(spec.id) ? 'Show ' : 'Close ') + spec.title,
      onclick: () => { if (hidden.has(spec.id)) workspace.show(spec.id); else workspace.hide(spec.id); dialog.close(); },
    })));
  body.append(el('button.btn', { text: 'Reset workspace', onclick: () => { workspace.reset(); dialog.close(); } }));
  body.append(savedLayouts(workspace, () => dialog.close()));
...
export function panelMenu(workspace, id) {
  ...
  const body = el('div.col', {}, [el('label', { text: 'Destination panel' }, [target]), el('label', { text: 'Position' }, [edge]),
    el('button.btn', { text: 'Move panel', disabled: !target.options.length,
