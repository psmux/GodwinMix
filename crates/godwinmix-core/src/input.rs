...
/// The same lines on Windows, with one difference worth saying out loud.
///
...
fn exec_process(spec: &ExecSpec, stdout: std::process::Stdio) -> Result<std::process::Child> {
    ...
    // Its own process group, so killing the source kills whatever it spawned.
    // Windows has no equivalent that costs nothing: the same guarantee wants a
    // Job Object with JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE, which is a handle to
    // own and close and is not worth adding until an exec source on Windows is
    // something people actually run. Until then a `--exec` command that spawns
    // children of its own can leave grandchildren behind on Windows, and
    // `descendants` returns nothing there to find them with.
    #[cfg(unix)]
    {
...
mod tests {
    ...
    /// once.
    // Ignored on 2026-09-15: the count is process wide and the workspace suite
// now runs more than six hundred tests beside it, holding sockets and
// pipelines for longer than any wait here. It passes alone, which is how it
// is meant to be read: `cargo test -p godwinmix-core --lib a_source_that_comes_and_goes_leaves_no_descriptors_behind -- --ignored`.
// The soak script (`dev/soak.sh`) watches descriptors on a real core, which is
// the check that matters for a show.
#[ignore]
#[test]
    #[cfg(unix)]
...
// now runs more than six hundred tests beside it, holding sockets and
// pipelines for longer than any wait here. It passes alone, which is how it
// is meant to be read: `cargo test -p godwinmix-core --lib probing_a_page_leaves_no_descriptors_behind -- --ignored`.
// The soak script (`dev/soak.sh`) watches descriptors on a real core, which is
// the check that matters for a show.
#[ignore]
#[test]
