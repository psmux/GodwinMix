...
impl Mixer {
    ...
    fn hold_encoder_for(&mut self, id: &OutputId) {
        self.output_leases
            .insert(id.clone(), self.enc.lease("output"));
        self.sync_encoder();
    }
    ...
    fn open_local_preview(&mut self, target: &str) -> Result<String, String> {
        ...
        let dir = self.runtime_dir.clone().ok_or_else(|| {
            "this core has no runtime directory, so it cannot place a preview socket".to_string()
        })?;
