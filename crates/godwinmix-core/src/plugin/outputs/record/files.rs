...
pub fn settings(params: &Params) -> Result<(PathBuf, String)> {
    let format = params
        ...
        .unwrap_or("mp4");
    anyhow::ensure!(
        matches!(format, "mp4" | "mkv"),
        "record/output format must be mp4 or mkv; choose one before starting"
    );
...
pub fn reserve(folder: &Path, id: &str, format: &str) -> Result<PathBuf> {
    ...
    std::fs::create_dir_all(folder).with_context(|| {
        format!(
            "cannot create {}; choose a writable recording folder",
            folder.display()
        )
    })?;
            ...
            .open(&path)
        {
            ...
            Err(e) => {
                return Err(e).with_context(|| {
                    format!(
                        "cannot write {}; choose a writable recording folder",
                        path.display()
                    )
                })
            }
