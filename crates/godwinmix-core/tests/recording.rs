...
fn recording_files_decode_and_restarts_preserve_existing_footage() {
    ...
    for format in ["mp4", "mkv"] {
        ...
        cfg.params.insert("directory".into(), folder.to_string_lossy().to_string().into());
        ...
        for generation in 0..2 {
            ...
            output.build(&OutputCtx { id: &cfg.id, generation, pipeline: &pipeline, params: &params, cfg: &cfg },
                &pipeline.by_name("video").unwrap(), &pipeline.by_name("audio").unwrap()).unwrap();
            pipeline.set_state(gst::State::Playing).unwrap();
            let message = pipeline.bus().unwrap().timed_pop_filtered(gst::ClockTime::from_seconds(20),
                &[gst::MessageType::Eos, gst::MessageType::Error]).expect("recorder completes");
            if let gst::MessageView::Error(error) = message.view() { panic!("{} {:?}", error.error(), error.debug()); }
    ...
    assert_eq!(std::fs::read(folder.join("existing.mp4")).unwrap(), b"keep me");
    std::fs::remove_dir_all(folder).unwrap();
}

fn decode(path: &str) {
    let uri = glib::filename_to_uri(path, None).unwrap();
    let play = gst::ElementFactory::make("playbin").property("uri", uri.as_str()).build().unwrap();
    ...
    let message = play.bus().unwrap().timed_pop_filtered(gst::ClockTime::from_seconds(15),
        &[gst::MessageType::Eos, gst::MessageType::Error]).expect("recorded file decodes to its end");
...
fn invalid_recording_format_has_a_next_step() {
    ...
    let error = output::open(&cfg).err().expect("unsupported format is refused");
