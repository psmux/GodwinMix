...
async fn removing_a_recording_finishes_a_playable_file_without_stopping_the_mixer() {
    ...
    tokio::spawn(async move {
        while let Some(event) = bus_rx.recv().await {
            if bus_handle.send(Command::Bus(event)).is_err() {
                break;
            }
        }
    });
    ...
    output.params.insert(
        "directory".into(),
        folder.to_string_lossy().to_string().into(),
    );
    handle
        .request(|ack| Command::AddOutput(Box::new(output), Some(ack)))
        .await
        .unwrap();
    ...
    handle
        .request(|ack| Command::RemoveOutput("archive".into(), Some(ack)))
        .await
        .unwrap();
    assert!(
        start.elapsed() < Duration::from_secs(2),
        "file drain blocked the control queue"
    );
    assert!(handle.status().await.unwrap().outputs.is_empty());
    tokio::time::sleep(Duration::from_secs(2)).await;
    let info = gstreamer_pbutils::Discoverer::new(gstreamer::ClockTime::from_seconds(5))
        .unwrap()
        .discover_uri(glib::filename_to_uri(&path, None).unwrap().as_str())
        .unwrap();
    assert!(
        !info.video_streams().is_empty(),
        "the file has no playable video"
    );
    assert!(
        !info.audio_streams().is_empty(),
        "the file has no playable audio"
    );
