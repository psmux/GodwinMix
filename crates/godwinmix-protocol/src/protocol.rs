...
pub fn ext_table() -> Vec<(&'static str, &'static str, &'static str, bool)> {
    vec![
        ...
        (
            "agent",
            "true or thresholds",
            "event/agent.state with a snapshot URL",
            false,
        ),
    ]
}
...
pub const WELL_KNOWN: &[(&str, &str, &str)] = &[
    ...
    (
        "GET",
        "/rpc",
        "the JSON-RPC WebSocket. Everything in `methods` is reachable here.",
    ),
    (
        "GET",
        "/api/v1/status",
        "an alias for GET /api/v1/core/status, because it is what people type",
    ),
    (
        "ANY",
        "/api/v1/{*rest}",
        "every method's REST route, generated by the transform rule",
    ),
];
...
fn method_entry<C>(m: &crate::method::MethodDef<C>, g: &mut SchemaGenerator) -> Value {
    ...
    if let Some(rest) = &m.rest {
        entry.insert(
            "rest".into(),
            json!({ "method": rest.http, "path": rest.path }),
        );
    }
...
fn markdown_envelope(doc: &Value, out: &mut String) {
    ...
    for (key, spec) in doc["envelope"]["properties"]
        .as_object()
        .into_iter()
        .flatten()
    {
...
mod tests {
    ...
    fn the_published_ext_table_matches_what_the_subscription_struct_reads() {
        let implemented: Vec<&str> = ext_table()
            .into_iter()
            .filter(|(_, _, _, yes)| *yes)
            .map(|(k, _, _, _)| k)
            .collect();
        assert_eq!(
            implemented,
            vec!["multiview", "meters", "tally", "positions"]
        );
        ...
        .unwrap();
        assert!(
            ext.unsupported().is_empty(),
            "a published key landed in `other`"
        );
    }
}
